...
Если **`docker compose up`** падает с **`port is already allocated`**: на
хосте уже слушает кто-то **`127.0.0.1:8080`** или **`127.0.0.1:8083`**
(часто старый контейнер после смены `docker-compose.yml` или второй стек).
Проверка: `docker ps --format 'table {{.Names}}\t{{.Ports}}'`, затем
`ss -tlnp | grep -E ':8080|:8083'`. В каталоге проекта выполните
`docker compose down`, при необходимости `docker stop …` для контейнера из
колонки PORTS. Альтернатива: в `.env` задайте свободный **`WEB_HOST_PORT`**
(одно число) и обновите **`proxy_pass`** в `infra-proxy` для этого домена.
...
services:
  ...
  web:
    ports:
      - target: 3000
        host_ip: 127.0.0.1
        published: "8083"   # или ${WEB_HOST_PORT:-8083}
        protocol: tcp
```

### 4.3 `/opt/xstore` — xstore55.ru

Аналогично `xstore-buy`, но по умолчанию **`WEB_HOST_PORT=8080`** (витрина на
`127.0.0.1:8080` → контейнер `3000`).
